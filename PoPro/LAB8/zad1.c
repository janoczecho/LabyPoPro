...
/* Polecenie 4.: Program posiada błędy związane z obsługą nieprawidłowych opcji menu w kontekście pustej listy. 
Opcje, takie jak wyświetlanie elementów (w), usuwanie elementu (u), czy usuwanie całej listy (z), mogą być wybrane, 
gdy lista jest pusta, co prowadzi do nieoczekiwanego zachowania, np. wyświetlania pustej listy lub próby usunięcia 
elementu z pustej listy. Należy dodać odpowiednie sprawdzenie stanu listy przed wywołaniem tych opcji oraz odpowiednie 
komunikaty o błędach, gdy lista jest pusta. Ponadto, funkcja odczytująca opcję menu powinna być poprawiona, aby skutecznie 
oczyszczać bufor wejściowy i unikać błędów związanych z długimi danymi wejściowymi.
*/